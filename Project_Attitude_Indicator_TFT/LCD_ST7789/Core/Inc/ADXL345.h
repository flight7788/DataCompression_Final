...
typedef struct Activites {
    uint8_t isOverrun;
    uint8_t isWatermark;
    uint8_t isFreeFall;
    uint8_t isInactivity;
    uint8_t isActivity;
    uint8_t isActivityOnX;
    uint8_t isActivityOnY;
    uint8_t isActivityOnZ;
    uint8_t isDoubleTap;
    uint8_t isTap;
    uint8_t isTapOnX;
    uint8_t isTapOnY;
...
Vector ADXL345_readNormalize(double gravityFactor); //= ADXL345_GRAVITY_EARTH
Vector ADXL345_readScaled(void);

Activites readActivites(void);

Vector ADXL345_lowPassFilter(Vector vector, double alpha);	//double alpha = 0.5
...
double ADXL345_getDoubleTapLatency(void);
