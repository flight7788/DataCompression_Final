...
uint16_t WindowsXstart = 0;
uint16_t WindowsYstart = 0;
uint16_t WindowsXend = ST7789H2_LCD_PIXEL_WIDTH-1;
uint16_t WindowsYend = ST7789H2_LCD_PIXEL_HEIGHT-1;

ST7789H2_Rgb888 ST7789H2_ReadPixel_rgb888(uint16_t Xpos, uint16_t Ypos);
...
void ST7789H2_Init(void) {
  ...
  ST7789H2_WriteReg(ST7789H2_TEARING_EFFECT, parameter, 1);
	for(uint32_t h = 0;h < ST7789H2_LCD_PIXEL_HEIGHT; h++) {
		for(uint16_t w = 0; w < ST7789H2_LCD_PIXEL_WIDTH; w++) {
			ST7789H2_WritePixel(w, h, 0);
		}
	}
}
...
void ST7789H2_SetDisplayWindow(uint16_t Xpos, uint16_t Ypos, uint16_t Width, uint16_t Height) {
  ...
  if (Height + Ypos <= ST7789H2_LCD_PIXEL_HEIGHT) {
	...
	uint8_t xa[] = { WindowsXstart >> 16, WindowsXstart & 0xffff, WindowsXend >> 16, WindowsXend & 0xffff}, 
	        ya[] = { WindowsYstart >> 16, WindowsYstart & 0xffff, WindowsYend >> 16, WindowsYend & 0xffff};
  //xa = ((uint32_t)x << 16) | (x+w-1);
  //ya = ((uint32_t)y << 16) | (y+h-1);
  
	ST7789H2_WriteReg(ST7789H2_CASET, xa, 4);
	ST7789H2_WriteReg(ST7789H2_RASET, ya, 4);
