...
R_u_e = u_e(:,:,3);
min(min(min(double(R_u_e))))
max(max(max(double(R_u_e))))
...
min(min(min(error_mapping)))
max(max(max(error_mapping)))

error_mapping = round(error_mapping);
re_error = error_mapping - abs(min_e);
re_error = error_mapping*2-255;
min(min(min(re_error)))
max(max(max(re_error)))
e_shift = error + abs(min(min(min(error))));
min(min(min(e_shift)))
max(max(max(e_shift)))

distrubution = zeros(1,255*2 + 1);
for i = 1:size(error,3)
   for j = 1:size(error,1)
        for k = 1:size(error,2)
            distrubution(double(u_e(j,k,i)) + 256) = distrubution(double(u_e(j,k,i)) + 256) + 1;
        end 
   end
end
figure;plot( -255:255, distrubution)



re = double(J_full) + error;
MSE = sum(sum(sum((input-re).^2))) / (512*512*3);

imshow(uint8(R_e_shift));
